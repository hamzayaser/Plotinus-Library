...
export default function Kutuphane({ sources = [], error }) {
  ...
  /*
   * ============================================================
   * COinS (Zotero Web Connector için)
   * ============================================================
   */
  ...
  const getCOinSFormat = (source) => {
    ...
    // Başlık Eşlemesi (Makalelerde atitle, Kitap/Bölümlerde btitle)
    if (source.baslik) {
      if (entryType === 'article') {
        params.set('rft.atitle', source.baslik);
      } else {
        params.set('rft.btitle', source.baslik);
      }
    }
    ...
    // Dergi adı / kitap adı / ansiklopedi adı / bildiri kitabı -> kaynak_adi
    ...
    // Baskı / Edisyon Sayısı (İsnad Uyumlu)
    if (source.baski) {
      params.set('rft.edition', String(source.baski));
    }
    ...
    authors.forEach((author) => {
      ...
      if (fullName) {
        ...
        if (author.lastName) params.append('rft.aulast', author.lastName);
        if (author.firstName) params.append('rft.aufirst', author.firstName);
      }
    });

    // Çevirmenler (Düzeltildi: rft.au yerine COinS standardı olan rft.ttr kullanıldı)
    ...
    translators.forEach((translator) => {
      const fullName =
        translator.name ||
        [translator.firstName, translator.lastName].filter(Boolean).join(' ');

      if (fullName) {
        params.append('rft.ttr', fullName);
      }
    });

    // Editör / Tahkik Eden / Hazırlayan (Düzeltildi: rft.aucorp yerine rft.edt kullanıldı)
    ...
    editors.forEach((editor) => {
      ...
      if (fullName) {
        params.append('rft.edt', fullName);
      }
    });
  ...
  const createBibTeX = (source) => {
    ...
    // Baskı / Edisyon Sayısı (BibTeX edition alanı)
    if (source.baski) {
      bibtex += `  edition = {${escapeBibTeX(
        source.baski
      )}},\n`;
    }
